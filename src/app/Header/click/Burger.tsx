'use client'
import DarkLight from '@/app/DarkLight'
import Link from 'next/link'
import React from 'react'

function burger(e: React.MouseEvent<HTMLDivElement>) {
    let a = e.currentTarget.children
    let b = a[0]
    let c = a[1]
    let d = e.currentTarget.nextElementSibling
    b.classList.toggle('rotate-[44deg]')
    b.classList.toggle('origin-left')
    b.classList.toggle('translate-x-[20%]')
    c.classList.toggle('-rotate-[44deg]')
    c.classList.toggle('origin-left')
    c.classList.toggle('translate-x-[20%]')
    d?.classList.toggle('hidden')
}
/////////////////////////////////////////////////////////////////
function rot_block(e: React.MouseEvent<HTMLButtonElement>) {
    let Child = e.currentTarget.children
    let next = e.currentTarget.nextElementSibling
    Child[0].classList.toggle('rotate-90')
    let stat = next?.getAttribute('data-stat')
    if (stat == 'off') {
        next?.classList.add('h-[40vh]')
        next?.classList.add('overflow-y-scroll')
        next?.classList.remove('h-0')
        next?.classList.remove('overflow-hidden')
        next?.setAttribute('data-stat', 'on')
    } else {
        next?.classList.add('h-0')
        next?.classList.add('overflow-hidden')
        next?.classList.remove('h-[40vh]')
        next?.classList.remove('overflow-y-scroll')
        next?.setAttribute('data-stat', 'off')
    }
}
export default function Burger() {
    return (
        <>
            <div onClick={burger} className='cursor-pointer border border-gray-800 py-1.5 px-2.5 rounded-4xl z-40 flex flex-col items-center justify-center w-[30px] h-[30px] text-sm hover:bg-[#352d2d] '>
                <div className='w-3.25 h-px bg-white transition-all z-20'></div>
                <div className='w-3.25 h-px bg-white mt-2 transition-all z-20'></div>
            </div>
            <div className='w-full fixed top-0 right-0 min-h-[150vh]  bg-black border hidden'>
                <section className='w-full flex flex-wrap justify-center mt-[12vh] '>
                    <Link className='border w-[92%] flex justify-center items-center border-gray-800 py-2 px-2.5 rounded-md text-sm hover:bg-[#e4d0d0] bg-white text-black' href=''>Sign Up</Link>ّ
                    <Link className='border w-[92%] flex justify-center items-center bg-gray-950 border-gray-800 py-2 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3' href=''>Log In</Link>
                    <div className='w-[92%]'>
                        <button onClick={rot_block} className='w-full text-[16px] text-[#A1A1A1A1] hover:text-white flex justify-between items-center bg-black h-11 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3'>
                            Product
                            <span className=''>
                                <svg viewBox="0 0 16 16" height="16" width="16"><path fill="currentColor" fillRule="evenodd" d="m5.5 1.94.53.53 4.82 4.82a1 1 0 0 1 0 1.42l-4.82 4.82-.53.53L4.44 13l.53-.53L9.44 8 4.97 3.53 4.44 3z" clipRule="evenodd"></path></svg>
                            </span>
                        </button>
                        <ul data-stat='off' className='h-0 scrollbar-thin scrollbar-[#A1A1A1A1] scrollbar- transition-all w-full overflow-hidden'>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75 ' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16" ><path d="M2.5 0.5V0H3.5V0.5C3.5 1.60457 4.39543 2.5 5.5 2.5H6V3V3.5H5.5C4.39543 3.5 3.5 4.39543 3.5 5.5V6H3H2.5V5.5C2.5 4.39543 1.60457 3.5 0.5 3.5H0V3V2.5H0.5C1.60457 2.5 2.5 1.60457 2.5 0.5Z" fill="currentColor"></path>
                                    <path d="M14.5 4.5V5H13.5V4.5C13.5 3.94772 13.0523 3.5 12.5 3.5H12V3V2.5H12.5C13.0523 2.5 13.5 2.05228 13.5 1.5V1H14H14.5V1.5C14.5 2.05228 14.9477 2.5 15.5 2.5H16V3V3.5H15.5C14.9477 3.5 14.5 3.94772 14.5 4.5Z" fill="currentColor"></path>
                                    <path d="M8.40706 4.92939L8.5 4H9.5L9.59294 4.92939C9.82973 7.29734 11.7027 9.17027 14.0706 9.40706L15 9.5V10.5L14.0706 10.5929C11.7027 10.8297 9.82973 12.7027 9.59294 15.0706L9.5 16H8.5L8.40706 15.0706C8.17027 12.7027 6.29734 10.8297 3.92939 10.5929L3 10.5V9.5L3.92939 9.40706C6.29734 9.17027 8.17027 7.29734 8.40706 4.92939Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>AI Gateway</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75 ' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16" ><path fillRule="evenodd" clipRule="evenodd" d="M5.25 0.0986328L5.66601 0.375964L10.166 3.37584L10.5 3.59849V3.99989V5.00001H9.55173L6 7.17051V12.25V13.5873L4.85891 12.89L0.358912 10.14L0 9.92064V9.50001V3.99989V3.59849L0.333987 3.37584L4.83399 0.375964L5.25 0.0986328ZM5.25 5.87093L8.35666 3.97241L5.25 1.90139L2.14334 3.97241L5.25 5.87093ZM1.5 5.33718L4.5 7.17051V10.9127L1.5 9.07938V5.33718ZM13.2307 12C13.2 12.815 13.0938 13.6278 12.9124 14.4279C13.8564 13.9717 14.5462 13.0724 14.7118 12H13.2307ZM11.8047 14.7359C11.7044 14.7452 11.6028 14.75 11.5 14.75C11.3972 14.75 11.2956 14.7452 11.1953 14.7359C10.9494 13.839 10.8077 12.9211 10.77 12H12.23C12.1923 12.9211 12.0506 13.839 11.8047 14.7359ZM13.2307 11C13.2 10.185 13.0938 9.37224 12.9124 8.57213C13.8564 9.02834 14.5462 9.92764 14.7118 11H13.2307ZM12.23 11C12.1923 10.0789 12.0506 9.16097 11.8047 8.2641C11.7044 8.25477 11.6028 8.25 11.5 8.25C11.3972 8.25 11.2956 8.25477 11.1953 8.2641C10.9494 9.16097 10.8077 10.0789 10.77 11H12.23ZM9.76925 11C9.80005 10.185 9.90616 9.37224 10.0876 8.57213C9.1436 9.02834 8.45381 9.92764 8.28822 11H9.76925ZM10.0876 14.4279C9.90616 13.6278 9.80005 12.815 9.76925 12H8.28822C8.45381 13.0724 9.1436 13.9717 10.0876 14.4279ZM11.5 16C13.9853 16 16 13.9853 16 11.5C16 9.01472 13.9853 7 11.5 7C9.01472 7 7 9.01472 7 11.5C7 13.9853 9.01472 16 11.5 16Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Sandbox</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75 ' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16" ><g fill="currentColor"><path d="m1.87061 12.709.50878.291v.6318.46h-1.800777l.913087-1.5986zm2.54394-.1172v1.5h-1.01758v-1.5zm2.03516 0v1.5h-1.01758v-1.5zm2.03515 0v1.5h-1.01757v-1.5zm2.03514 0v1.5h-1.01756v-1.5zm2.0352 0v1.5h-1.0176v-1.5zm2.8183 1.5h-1.8007v-.46-.6328l.5087-.29.378-.2158zm-11.48922-2.7627-.54492.9541-1.30274-.7441.5459-.9541zm10.03032.21-1.3028.7441-.5449-.9541 1.3027-.7441zm-9.21196-1.64066-.27344.47656-1.30176-.74414.27246-.47656zm8.12206-.26758-1.3027.74414-.2725-.47656 1.3027-.74414z" /><path d="m12.4438 8.96362-1.3027.74414-3.16503-5.53906-3.16406 5.53906-1.30273-.74414 4.46679-7.81738z" /><path d="m12.1201 6.25586c.9134.15439 1.7896.3717 2.3819.63672.2959.13244.5807.29676.8027.50586.2222.20928.4453.5284.4453.94922-.0001.67816-.5565 1.08681-.9346 1.29785-.4413.24632-1.0302.44409-1.6933.60059-1.3382.3159-3.14876.5039-5.1221.5039s-3.78392-.188-5.12207-.5039c-.66312-.1565-1.25202-.35427-1.69336-.60059-.354472-.19786-.866169-.56934-.928711-1.17383l-.005859-.12402.009766-.15332c.045815-.34573.241161-.61281.435546-.7959.222012-.2091.506778-.37342.802738-.50586.59356-.26559 1.39584-.47552 2.31054-.62988l-1.00195 1.7539c-.2744.07842-.50726.161-.69531.24512-.06657.02979-.12403.05857-.17285.08496.26877.14551.69695.30083 1.28418.43945 1.19534.28217 2.88475.46387 4.77734.46387s3.582-.1817 4.7773-.46387c.5867-.13849 1.0144-.29406 1.2832-.43945-.0486-.02628-.1056-.05533-.1718-.08496-.2059-.09209-.4654-.18261-.7744-.26758z" /></g></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Vercel Agent</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75 ' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M8 0.154663L8.34601 0.334591L14.596 3.58459L15 3.79466V4.25V11.75V12.2053L14.596 12.4154L8.34601 15.6654L8 15.8453L7.65399 15.6654L1.40399 12.4154L1 12.2053V11.75V4.25V3.79466L1.40399 3.58459L7.65399 0.334591L8 0.154663ZM2.5 11.2947V5.44058L7.25 7.81559V13.7647L2.5 11.2947ZM8.75 13.7647L13.5 11.2947V5.44056L8.75 7.81556V13.7647ZM8 1.84534L12.5766 4.22519L7.99998 6.51352L3.42335 4.2252L8 1.84534Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>AI SDK</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75 ' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path d="M6.0952 9.4643V5.5238H7.6190V10.5476C7.6190 11.1394 7.1394 11.6190 6.5476 11.6190C6.2651 11.6190 5.9862 11.5101 5.7857 11.3096L0 5.5238H2.1548L6.0952 9.4643Z M16 10.0952H14.4762V6.6071L10.9881 10.0952H14.4762V11.6190H10.5238C9.3403 11.6190 8.3810 10.6597 8.3810 9.4762V5.5238H9.9048V9.0238L13.4047 5.5238H9.9048V4H13.8571C15.0407 4 16 4.9594 16 6.1429V10.0952Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>vO</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75 ' viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M14.5 2.5H1.5V5.005H14.5V2.5ZM14.5 6.255H6.245V13.5H13.5C14.0523 13.5 14.5 13.0523 14.5 12.5V6.255ZM4.995 6.255H1.5V12.5C1.5 13.0523 1.94772 13.5 2.5 13.5H4.995V6.255ZM1.5 1H0V2.5V12.5C0 13.8807 1.11929 15 2.5 15H13.5C14.8807 15 16 13.8807 16 12.5V2.5V1H14.5H1.5Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>CI/CD</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M10.268 14.0934C11.9051 13.4838 13.2303 12.2333 13.9384 10.6469C13.1192 10.7941 12.2138 10.9111 11.2469 10.9925C11.0336 12.2005 10.695 13.2621 10.268 14.0934ZM8 16C12.4183 16 16 12.4183 16 8C16 3.58172 12.4183 0 8 0C3.58172 0 0 3.58172 0 8C0 12.4183 3.58172 16 8 16ZM8.48347 14.4823C8.32384 14.494 8.16262 14.5 8 14.5C7.83738 14.5 7.67616 14.494 7.51654 14.4823C7.5132 14.4791 7.50984 14.4759 7.50647 14.4726C7.2415 14.2165 6.94578 13.7854 6.67032 13.1558C6.41594 12.5744 6.19979 11.8714 6.04101 11.0778C6.67605 11.1088 7.33104 11.125 8 11.125C8.66896 11.125 9.32395 11.1088 9.95899 11.0778C9.80021 11.8714 9.58406 12.5744 9.32968 13.1558C9.05422 13.7854 8.7585 14.2165 8.49353 14.4726C8.49016 14.4759 8.4868 14.4791 8.48347 14.4823ZM11.4187 9.72246C12.5137 9.62096 13.5116 9.47245 14.3724 9.28806C14.4561 8.87172 14.5 8.44099 14.5 8C14.5 7.55901 14.4561 7.12828 14.3724 6.71194C13.5116 6.52755 12.5137 6.37904 11.4187 6.27753C11.4719 6.83232 11.5 7.40867 11.5 8C11.5 8.59133 11.4719 9.16768 11.4187 9.72246ZM10.1525 6.18401C10.2157 6.75982 10.25 7.36805 10.25 8C10.25 8.63195 10.2157 9.24018 10.1525 9.81598C9.46123 9.85455 8.7409 9.875 8 9.875C7.25909 9.875 6.53877 9.85455 5.84749 9.81598C5.7843 9.24018 5.75 8.63195 5.75 8C5.75 7.36805 5.7843 6.75982 5.84749 6.18401C6.53877 6.14545 7.25909 6.125 8 6.125C8.74091 6.125 9.46123 6.14545 10.1525 6.18401ZM11.2469 5.00748C12.2138 5.08891 13.1191 5.20593 13.9384 5.35306C13.2303 3.7667 11.9051 2.51622 10.268 1.90662C10.695 2.73788 11.0336 3.79953 11.2469 5.00748ZM8.48347 1.51771C8.4868 1.52089 8.49016 1.52411 8.49353 1.52737C8.7585 1.78353 9.05422 2.21456 9.32968 2.84417C9.58406 3.42562 9.80021 4.12856 9.95899 4.92219C9.32395 4.89118 8.66896 4.875 8 4.875C7.33104 4.875 6.67605 4.89118 6.04101 4.92219C6.19978 4.12856 6.41594 3.42562 6.67032 2.84417C6.94578 2.21456 7.2415 1.78353 7.50647 1.52737C7.50984 1.52411 7.51319 1.52089 7.51653 1.51771C7.67615 1.50597 7.83738 1.5 8 1.5C8.16262 1.5 8.32384 1.50597 8.48347 1.51771ZM5.73202 1.90663C4.0949 2.51622 2.76975 3.7667 2.06159 5.35306C2.88085 5.20593 3.78617 5.08891 4.75309 5.00748C4.96639 3.79953 5.30497 2.73788 5.73202 1.90663ZM4.58133 6.27753C3.48633 6.37904 2.48837 6.52755 1.62761 6.71194C1.54392 7.12828 1.5 7.55901 1.5 8C1.5 8.44099 1.54392 8.87172 1.62761 9.28806C2.48837 9.47245 3.48633 9.62096 4.58133 9.72246C4.52807 9.16768 4.5 8.59133 4.5 8C4.5 7.40867 4.52807 6.83232 4.58133 6.27753ZM4.75309 10.9925C3.78617 10.9111 2.88085 10.7941 2.06159 10.6469C2.76975 12.2333 4.0949 13.4838 5.73202 14.0934C5.30497 13.2621 4.96639 12.2005 4.75309 10.9925Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Content Delivery</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M7 2H9V0.75V0H10.5V0.75V2H13C13.5523 2 14 2.44772 14 3V5.5H15.25H16V7H15.25H14V9H15.25H16V10.5H15.25H14V13C14 13.5523 13.5523 14 13 14H10.5V15.25V16H9V15.25V14H7V15.25V16H5.5V15.25V14H3C2.44772 14 2 13.5523 2 13V10.5H0.75H0V9H0.75H2V7H0.75H0V5.5H0.75H2V3C2 2.44772 2.44772 2 3 2H5.5V0.75V0H7V0.75V2ZM3.5 8.98228V3.5H12.5V9H10C9.56114 9 9.29513 8.85208 9.13685 8.68588C8.96919 8.50984 8.875 8.26309 8.875 8C8.875 7.73691 8.96919 7.49016 9.13685 7.31412C9.29513 7.14792 9.56114 7 10 7V5.5C8.82792 5.5 7.9118 5.74294 7.16034 6.13019C6.41599 6.51379 5.87229 7.01955 5.42887 7.4794C5.29221 7.62111 5.17056 7.75171 5.05789 7.87267L5.05789 7.87267C4.78031 8.17066 4.55724 8.41014 4.2986 8.6132C4.04734 8.81045 3.80061 8.93775 3.5 8.98228ZM3.5 10.4907V12.5H12.5V10.5H10C9.18886 10.5 8.51737 10.2104 8.05065 9.72037C7.59331 9.24016 7.375 8.61191 7.375 8C7.375 7.91436 7.37928 7.8284 7.38788 7.74258C7.06615 7.96886 6.78522 8.23377 6.50863 8.5206C6.41912 8.61343 6.32548 8.71405 6.22828 8.8185L6.22819 8.8186C5.92315 9.14637 5.58298 9.5119 5.22484 9.79305C4.75982 10.1581 4.20727 10.437 3.5 10.4907Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Fluid Compute</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path d="M5.75 7L6.4209 7.33594L4.85254 10.4717C5.38789 10.771 5.75 11.3431 5.75 12V14C5.75 14.9665 4.96648 15.75 4 15.75H2C1.03352 15.75 0.25 14.9665 0.25 14V12C0.25 11.0335 1.03352 10.25 2 10.25H3.28711L5.0791 6.66504L5.75 7ZM14 10.25C14.9665 10.25 15.75 11.0335 15.75 12V14C15.75 14.9665 14.9665 15.75 14 15.75H12C11.1185 15.75 10.3909 15.098 10.2695 14.25H7V12.75H10.25V12C10.25 11.0335 11.0335 10.25 12 10.25H14ZM2 11.75C1.86192 11.75 1.75 11.8619 1.75 12V14C1.75 14.1381 1.86192 14.25 2 14.25H4C4.13808 14.25 4.25 14.1381 4.25 14V12C4.25 11.8619 4.13808 11.75 4 11.75H2ZM12 11.75C11.8619 11.75 11.75 11.8619 11.75 12V14C11.75 14.1381 11.8619 14.25 12 14.25H14C14.1381 14.25 14.25 14.1381 14.25 14V12C14.25 11.8619 14.1381 11.75 14 11.75H12ZM9 0.25C9.96649 0.25 10.75 1.03351 10.75 2V4C10.75 4.44642 10.5808 4.85197 10.3057 5.16113L12.2041 8.6416L10.8877 9.36035L8.91895 5.75H7C6.03351 5.75 5.25 4.96649 5.25 4V2C5.25 1.03351 6.03351 0.25 7 0.25H9ZM7 1.75C6.86193 1.75 6.75 1.86193 6.75 2V4C6.75 4.13807 6.86193 4.25 7 4.25H9C9.13807 4.25 9.25 4.13807 9.25 4V2C9.25 1.86193 9.13807 1.75 9 1.75H7Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Workflow</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M8.75 1V1.75V14.25V15H7.25V14.25V1.75V1H8.75ZM3.5 9V9.75V14.25V15H2V14.25V9.75V9H3.5ZM14 6.75V6H12.5V6.75V14.25V15H14V14.25V6.75Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Observability</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" className='rounded-sm h-8.75 w-8.75' strokeLinejoin="round" viewBox="-6 -1 30 16" width="16" ><path fillRule="evenodd" clipRule="evenodd" d="M8.75 2.79933C9.19835 2.53997 9.5 2.05521 9.5 1.5C9.5 0.671573 8.82843 0 8 0C7.17157 0 6.5 0.671573 6.5 1.5C6.5 2.05521 6.80165 2.53997 7.25 2.79933V5H7C4.027 5 1.55904 7.16229 1.08296 10H0V13H1V14.5V16H2.5H13.5H15V14.5V13H16V10H14.917C14.441 7.16229 11.973 5 9 5H8.75V2.79933ZM7 6.5C4.51472 6.5 2.5 8.51472 2.5 11V14.5H13.5V11C13.5 8.51472 11.4853 6.5 9 6.5H7ZM7.25 11.25C7.25 12.2165 6.4665 13 5.5 13C4.5335 13 3.75 12.2165 3.75 11.25C3.75 10.2835 4.5335 9.5 5.5 9.5C6.4665 9.5 7.25 10.2835 7.25 11.25ZM10.5 13C11.4665 13 12.25 12.2165 12.25 11.25C12.25 10.2835 11.4665 9.5 10.5 9.5C9.5335 9.5 8.75 10.2835 8.75 11.25C8.75 12.2165 9.5335 13 10.5 13Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Bot Managment</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path d="M1.5 10.5002V12.2502C1.5001 13.4928 2.50743 14.5002 3.75 14.5002H5.25V16.0002H3.75C1.67905 16.0002 0.000104111 14.3212 0 12.2502V10.5002H1.5ZM16 12.2502C15.9999 14.3212 14.321 16.0002 12.25 16.0002H10.75V14.5002H12.25C13.4926 14.5002 14.4999 13.4928 14.5 12.2502V10.5002H16V12.2502ZM8 2.00024C8.62132 2.00024 9.125 2.50392 9.125 3.12524C9.12494 3.45778 8.97942 3.75526 8.75 3.96118V5.00708C11.119 5.13681 13 7.09886 13 9.50024V12.0002C12.9999 12.5178 12.6066 12.944 12.1025 12.9954L12 13.0002H4C3.44778 13.0002 3.0001 12.5524 3 12.0002V9.50024C3 7.09886 4.88101 5.13681 7.25 5.00708V3.96118C7.02058 3.75526 6.87506 3.45778 6.875 3.12524C6.875 2.50392 7.37868 2.00024 8 2.00024ZM7.5 6.50024C5.84315 6.50024 4.5 7.84339 4.5 9.50024V11.5002H11.5V9.50024C11.5 7.84339 10.1569 6.50024 8.5 6.50024H7.5ZM6.5 8.50024C7.05228 8.50024 7.5 8.94796 7.5 9.50024C7.4999 10.0524 7.05222 10.5002 6.5 10.5002C5.94778 10.5002 5.5001 10.0524 5.5 9.50024C5.5 8.94796 5.94772 8.50024 6.5 8.50024ZM9.5 8.50024C10.0523 8.50024 10.5 8.94796 10.5 9.50024C10.4999 10.0524 10.0522 10.5002 9.5 10.5002C8.94778 10.5002 8.5001 10.0524 8.5 9.50024C8.5 8.94796 8.94771 8.50024 9.5 8.50024ZM5.25 1.50024H2.25C1.83579 1.50024 1.5 1.83603 1.5 2.25024V5.50024H0V2.25024C0 1.0076 1.00736 0.000244141 2.25 0.000244141H5.25V1.50024ZM13.75 0.000244141C14.9926 0.000244141 16 1.0076 16 2.25024V5.50024H14.5V2.25024C14.5 1.83603 14.1642 1.50024 13.75 1.50024H10.75V0.000244141H13.75Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>BotID</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><path fillRule="evenodd" clipRule="evenodd" d="M3.5 4.057V9.52717C3.5 10.9221 4.27429 12.2018 5.50997 12.849L8 14.1533L10.49 12.849C11.7257 12.2018 12.5 10.9221 12.5 9.52717V4.057C12.3094 4.00405 12.1074 3.9513 11.8932 3.89539C11.746 3.85699 11.5932 3.81709 11.4344 3.7746C10.8476 3.61758 10.204 3.43066 9.61101 3.17017C9.02666 2.91351 8.44336 2.56529 8 2.05704C7.55664 2.56529 6.97334 2.91351 6.38899 3.17017C5.79596 3.43066 5.15243 3.61758 4.5656 3.7746C4.40682 3.81709 4.25396 3.85699 4.10684 3.89539C3.89262 3.9513 3.69055 4.00405 3.5 4.057ZM7.25 0C7.25 0.467199 7.10537 0.796772 6.87802 1.06132C6.6357 1.34329 6.26955 1.58432 5.78576 1.79681C5.30375 2.00853 4.75351 2.17155 4.17787 2.32558C4.04421 2.36134 3.90727 2.39707 3.76932 2.43305C3.33687 2.54586 2.89458 2.66124 2.51283 2.78849L2 2.95943V3.5V9.52717C2 11.4801 3.084 13.2716 4.81396 14.1778L7.65199 15.6644L8 15.8467L8.34801 15.6644L11.186 14.1778C12.916 13.2716 14 11.4801 14 9.52717V3.5V2.95943L13.4872 2.78849C13.1054 2.66124 12.6631 2.54586 12.2307 2.43305C12.0927 2.39707 11.9558 2.36134 11.8221 2.32558C11.2465 2.17155 10.6962 2.00853 10.2142 1.79681C9.73045 1.58432 9.3643 1.34329 9.12198 1.06132C8.89463 0.796772 8.75 0.467199 8.75 0H7.25Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Platform Security</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="-6 -1 30 16" width="16"><g clipPath="url(#clip0_4629_2028)">
                                    <path fillRule="evenodd" clipRule="evenodd" d="M0 1.25C0 0.559645 0.559644 0 1.25 0H14.75C15.4404 0 16 0.559644 16 1.25V4.75V5.5H15.25H6V8.75V9.5H5.25H1.5V12H4.5V13.5H1.25C0.559644 13.5 0 12.9404 0 12.25V1.25ZM1.5 8H4.5V5.5H1.5V8ZM4.5 4H1.5V1.5H4.5V4ZM6 1.5V4H10V1.5H6ZM14.5 4H11.5V1.5H14.5V4ZM16 11.5C16 13.9853 13.9853 16 11.5 16C9.01472 16 7 13.9853 7 11.5C7 9.01472 9.01472 7 11.5 7C13.9853 7 16 9.01472 16 11.5ZM14.2861 10.3629L13.8442 10.8049L11.2407 13.4084C10.899 13.7501 10.345 13.7501 10.0033 13.4084L9.15574 12.5608L8.7138 12.1189L9.59768 11.235L10.0396 11.6769L10.622 12.2593L12.9603 9.92097L13.4023 9.47903L14.2861 10.3629Z" fill="currentColor"></path>
                                </g>
                                    <defs>
                                        <clipPath id="clip0_4629_2028">
                                            <rect width="16" height="16" fill="white"></rect>
                                        </clipPath>
                                        <mask id="path-1-inside-1_4629_2028" fill="white">
                                            <path fillRule="evenodd" clipRule="evenodd" d="M0 1.25C0 0.559645 0.559644 0 1.25 0H14.75C15.4404 0 16 0.559644 16 1.25V4.75V5.5H15.25H6V8.75V9.5H5.25H1.5V12H4.5V13.5H1.25C0.559644 13.5 0 12.9404 0 12.25V1.25ZM1.5 8H4.5V5.5H1.5V8ZM4.5 4H1.5V1.5H4.5V4ZM6 1.5V4H10V1.5H6ZM14.5 4H11.5V1.5H14.5V4ZM16 11.5C16 13.9853 13.9853 16 11.5 16C9.01472 16 7 13.9853 7 11.5C7 9.01472 9.01472 7 11.5 7C13.9853 7 16 9.01472 16 11.5ZM14.2861 10.3629L13.8442 10.8049L11.2407 13.4084C10.899 13.7501 10.345 13.7501 10.0033 13.4084L9.15574 12.5608L8.7138 12.1189L9.59768 11.235L10.0396 11.6769L10.622 12.2593L12.9603 9.92097L13.4023 9.47903L14.2861 10.3629Z"></path>
                                        </mask>
                                    </defs></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Web Application Firewall</h4>
                                </div>
                            </section>
                        </ul>
                        {/* ////////////////////////////////end pro////////////////////////////////////// */}
                    </div>
                    <div className='w-[92%]'>
                        <button onClick={rot_block} className='w-full text-[16px] text-[#A1A1A1A1] hover:text-white flex justify-between items-center bg-black h-11 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3'>
                            Resources
                            <span className=''>
                                <svg viewBox="0 0 16 16" height="16" width="16"><path fill="currentColor" fillRule="evenodd" d="m5.5 1.94.53.53 4.82 4.82a1 1 0 0 1 0 1.42l-4.82 4.82-.53.53L4.44 13l.53-.53L9.44 8 4.97 3.53 4.44 3z" clipRule="evenodd"></path></svg>
                            </span>
                        </button>
                        <ul data-stat='off' className='h-0 scrollbar-thin scrollbar-[#A1A1A1A1] scrollbar- transition-all w-full overflow-hidden'>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M14.5 8C14.5 11.5899 11.5899 14.5 8 14.5C4.41015 14.5 1.5 11.5899 1.5 8C1.5 4.41015 4.41015 1.5 8 1.5C11.5899 1.5 14.5 4.41015 14.5 8ZM16 8C16 12.4183 12.4183 16 8 16C3.58172 16 0 12.4183 0 8C0 3.58172 3.58172 0 8 0C12.4183 0 16 3.58172 16 8ZM11.5249 10.8478L11.8727 10.3286L10.8342 9.6329L10.4863 10.1522C9.94904 10.9543 9.0363 11.4802 8.00098 11.4802C6.96759 11.4802 6.05634 10.9563 5.51863 10.1567L5.16986 9.63804L4.13259 10.3356L4.48137 10.8542C5.2414 11.9844 6.53398 12.7302 8.00098 12.7302C9.47073 12.7302 10.7654 11.9816 11.5249 10.8478ZM6.75 6.75C6.75 7.30228 6.30228 7.75 5.75 7.75C5.19772 7.75 4.75 7.30228 4.75 6.75C4.75 6.19772 5.19772 5.75 5.75 5.75C6.30228 5.75 6.75 6.19772 6.75 6.75ZM10.25 7.75C10.8023 7.75 11.25 7.30228 11.25 6.75C11.25 6.19772 10.8023 5.75 10.25 5.75C9.69771 5.75 9.25 6.19772 9.25 6.75C9.25 7.30228 9.69771 7.75 10.25 7.75Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Customers</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M8.75 0.189331L9.28033 0.719661L15.2803 6.71966L15.8107 7.24999L15.2803 7.78032L13.7374 9.32322C13.1911 9.8696 12.3733 9.97916 11.718 9.65188L9.54863 13.5568C8.71088 15.0648 7.12143 16 5.39639 16H0.75H0V15.25V10.6036C0 8.87856 0.935237 7.28911 2.4432 6.45136L6.34811 4.28196C6.02084 3.62674 6.13039 2.80894 6.67678 2.26255L8.21967 0.719661L8.75 0.189331ZM7.3697 5.43035L10.5696 8.63029L8.2374 12.8283C7.6642 13.8601 6.57668 14.5 5.39639 14.5H2.56066L5.53033 11.5303L4.46967 10.4697L1.5 13.4393V10.6036C1.5 9.42331 2.1399 8.33579 3.17166 7.76259L7.3697 5.43035ZM12.6768 8.26256C12.5791 8.36019 12.4209 8.36019 12.3232 8.26255L12.0303 7.96966L8.03033 3.96966L7.73744 3.67677C7.63981 3.57914 7.63981 3.42085 7.73744 3.32321L8.75 2.31065L13.6893 7.24999L12.6768 8.26256Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Blog</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M6.28497 1.5H13V12C13 12.5523 12.5523 13 12 13H6.28497L6.28497 1.5ZM5.03497 1.5H3V12C3 12.5523 3.44772 13 4 13H5.03497L5.03497 1.5ZM5.03497 14.5H4C2.61929 14.5 1.5 13.3807 1.5 12V1.5V0H3H13H14.5V1.5V12C14.5 13.3807 13.3807 14.5 12 14.5H6.28497V15V15.625H5.03497V15V14.5ZM8.505 3.375H9.13H10.13H10.755V4.625H10.13H9.13H8.505V3.375ZM9.13 6.375H8.505V7.625H9.13H10.13H10.755V6.375H10.13H9.13Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Changelog</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M6 2.5H10C10.2761 2.5 10.5 2.72386 10.5 3V4H5.5V3C5.5 2.72386 5.72386 2.5 6 2.5ZM4 4V3C4 1.89543 4.89543 1 6 1H10C11.1046 1 12 1.89543 12 3V4H14.5H16V5.5V13.5C16 14.8807 14.8807 16 13.5 16H2.5C1.11929 16 0 14.8807 0 13.5V5.5V4H1.5H4ZM12 5.5H10.5H5.5H4H1.5V9.25H7.25V8.5H8.75V9.25L14.5 9.25V5.5H12ZM8.75 10.75L14.5 10.75V13.5C14.5 14.0523 14.0523 14.5 13.5 14.5H2.5C1.94772 14.5 1.5 14.0523 1.5 13.5V10.75H7.25V11.5H8.75V10.75Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Press</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M5.5 0.5V1.25V2H10.5V1.25V0.5H12V1.25V2H14H15.5V3.5V13.5C15.5 14.8807 14.3807 16 13 16H3C1.61929 16 0.5 14.8807 0.5 13.5V3.5V2H2H4V1.25V0.5H5.5ZM2 3.5H14V6H2V3.5ZM2 7.5V13.5C2 14.0523 2.44772 14.5 3 14.5H13C13.5523 14.5 14 14.0523 14 13.5V7.5H2Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Events</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M3.75 0C2.50736 0 1.5 1.00736 1.5 2.25V13.744V13.75H1.50001C1.50323 14.9899 2.50935 15.994 3.75 15.994H13H14.5V14.494V13.7296V12.994V11.494V0.75V0H13.75H3.75ZM13 11.494V1.5H3.75C3.33579 1.5 3 1.83579 3 2.25V11.622C3.23458 11.5391 3.48702 11.494 3.75 11.494H13ZM3 13.744C3 14.1582 3.33579 14.494 3.75 14.494H13V13.7296V12.994H3.75C3.33579 12.994 3 13.3298 3 13.744Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Docs</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M12.2803 0.719661L11.75 0.189331L11.2197 0.719661L1.09835 10.841C0.395088 11.5442 0 12.4981 0 13.4926V15.25V16H0.75H2.50736C3.50192 16 4.45575 15.6049 5.15901 14.9016L15.2803 4.78032L15.8107 4.24999L15.2803 3.71966L12.2803 0.719661ZM9.81066 4.24999L11.75 2.31065L13.6893 4.24999L11.75 6.18933L9.81066 4.24999ZM8.75 5.31065L2.15901 11.9016C1.73705 12.3236 1.5 12.8959 1.5 13.4926V14.5H2.50736C3.1041 14.5 3.67639 14.2629 4.09835 13.841L10.6893 7.24999L8.75 5.31065Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Academy</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M0 1H0.75H5C6.2267 1 7.31583 1.58901 8 2.49963C8.68417 1.58901 9.7733 1 11 1H15.25H16V1.75V13V13.75H15.25H10.7426C10.1459 13.75 9.57361 13.9871 9.15165 14.409L8.53033 15.0303H7.46967L6.84835 14.409C6.42639 13.9871 5.8541 13.75 5.25736 13.75H0.75H0V13V1.75V1ZM7.25 4.75C7.25 3.50736 6.24264 2.5 5 2.5H1.5V12.25H5.25736C5.96786 12.25 6.65758 12.4516 7.25 12.8232V4.75ZM8.75 12.8232V4.75C8.75 3.50736 9.75736 2.5 11 2.5H14.5V12.25H10.7426C10.0321 12.25 9.34242 12.4516 8.75 12.8232Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Knowledge Base</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16"><path fillRule="evenodd" clipRule="evenodd" d="M2.5 3.25C2.5 1.45507 3.95507 0 5.75 0H6.25C8.04493 0 9.5 1.45507 9.5 3.25V3.75C9.5 5.54493 8.04493 7 6.25 7H5.75C3.95507 7 2.5 5.54493 2.5 3.75V3.25ZM5.75 1.5C4.7835 1.5 4 2.2835 4 3.25V3.75C4 4.7165 4.7835 5.5 5.75 5.5H6.25C7.2165 5.5 8 4.7165 8 3.75V3.25C8 2.2835 7.2165 1.5 6.25 1.5H5.75ZM1.5 14.5V13.1709C2.31958 11.5377 3.99308 10.5 5.82945 10.5H6.17055C8.00692 10.5 9.68042 11.5377 10.5 13.1709V14.5H1.5ZM5.82945 9C3.35483 9 1.10604 10.4388 0.0690305 12.6857L0 12.8353V13V15.25V16H0.75H11.25H12V15.25V13V12.8353L11.931 12.6857C10.894 10.4388 8.64517 9 6.17055 9H5.82945ZM15.931 12.6857C15.3706 11.4715 14.4561 10.4931 13.3439 9.85058L12.5935 11.1494C13.399 11.6148 14.0681 12.3101 14.5 13.1709V14.5H13.5V16H15.25H16V15.25V13V12.8352L15.931 12.6857ZM11.25 0H10.5V1.5H11.25C12.2165 1.5 13 2.2835 13 3.25V3.75C13 4.7165 12.2165 5.5 11.25 5.5H10.5V7H11.25C13.0449 7 14.5 5.54493 14.5 3.75V3.25C14.5 1.45507 13.0449 0 11.25 0Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Community</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg className='rounded-sm h-8.75 w-8.75' height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16"><g clipPath="white">
                                    <circle cx="8" cy="8" r="7.375" fill="black" stroke="gray-1000" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"></circle>
                                    <path d="M10.63 11V5" className='stroke-gray-400' strokeWidth="1.25" strokeMiterlimit="1.41421"></path>
                                    <path fillRule="evenodd" clipRule="evenodd" d="M5.995 5.00087V5H4.745V11H5.995V6.96798L12.3615 14.7076C12.712 14.4793 13.0434 14.2242 13.353 13.9453L5.99527 5.00065L5.995 5.00087Z" fill="white"></path>
                                </g>
                                    <defs>
                                        <linearGradient x1="11.13" y1="5" x2="11.13" y2="11" gradientUnits="userSpaceOnUse">
                                            <stop stopColor="white"></stop>
                                            <stop offset="0.609375" stopColor="white" stopOpacity="0.57"></stop>
                                            <stop offset="0.796875" stopColor="white" stopOpacity="0"></stop>
                                            <stop offset="1" stopColor="white" stopOpacity="0"></stop>
                                        </linearGradient>
                                        <linearGradient x1="9.9375" y1="9.0625" x2="13.5574" y2="13.3992" gradientUnits="userSpaceOnUse">
                                            <stop stopColor="white"></stop>
                                            <stop offset="1" stopColor="white" stopOpacity="0"></stop>
                                        </linearGradient>
                                        <clipPath>
                                            <rect width="16" height="16" fill="red"></rect>
                                        </clipPath>
                                    </defs></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Next js</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M8.97614 13.3333H14.924C15.1129 13.3333 15.2985 13.2841 15.4621 13.1905C15.6257 13.0968 15.7616 12.9621 15.856 12.8C15.9504 12.6378 16.0001 12.4538 16 12.2666C15.9999 12.0794 15.9501 11.8955 15.8555 11.7334L11.8611 4.87625C11.7667 4.71411 11.6309 4.57946 11.4673 4.48585C11.3037 4.39225 11.1182 4.34295 10.9293 4.34295C10.7404 4.34295 10.5549 4.39225 10.3913 4.48585C10.2277 4.57946 10.0919 4.71411 9.99751 4.87625L8.97614 6.63074L6.97922 3.19987C6.88473 3.03776 6.74885 2.90313 6.58524 2.80953C6.42162 2.71594 6.23604 2.66666 6.04713 2.66666C5.85822 2.66666 5.67264 2.71594 5.50903 2.80953C5.3454 2.90313 5.20953 3.03776 5.11504 3.19987L0.144471 11.7334C0.0499099 11.8955 8.22996e-05 12.0794 1.01863e-07 12.2666C-8.20958e-05 12.4538 0.0495838 12.6378 0.144003 12.8C0.238421 12.9621 0.374263 13.0968 0.537867 13.1905C0.701468 13.2841 0.887063 13.3333 1.07598 13.3333H4.80956C6.28885 13.3333 7.37977 12.6893 8.13042 11.4329L9.95287 8.3048L10.929 6.63074L13.8586 11.6593H9.95287L8.97614 13.3333ZM4.74869 11.6575L2.14313 11.657L6.04887 4.95264L7.99769 8.3048L6.69287 10.5453C6.19436 11.3605 5.62804 11.6575 4.74869 11.6575Z" fill="#00DC82"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Nuxt</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M13.7974 2.11525C12.3195 -0.0135593 9.37709 -0.637288 7.26183 0.705085L3.53302 3.09153C2.51607 3.72881 1.81098 4.77288 1.60759 5.95254C1.43132 6.94237 1.58048 7.95932 2.05505 8.84068C1.72963 9.32881 1.51268 9.87119 1.41776 10.4407C1.20082 11.6475 1.48556 12.8949 2.19065 13.8847C3.68217 16.0136 6.61098 16.6373 8.72624 15.2949L12.4551 12.922C13.472 12.2847 14.1771 11.2407 14.3805 10.061C14.5567 9.07119 14.4076 8.05424 13.933 7.17288C14.2584 6.68475 14.4754 6.14237 14.5703 5.57288C14.8008 4.35254 14.5161 3.10508 13.7974 2.11525Z" fill="#FF3E00"></path>
                                    <path d="M6.8958 14.0881C5.68902 14.4 4.428 13.9254 3.72292 12.9085C3.28902 12.3119 3.12631 11.5661 3.24834 10.8339C3.27546 10.7119 3.30258 10.6034 3.3297 10.4814L3.3975 10.2644L3.58733 10.4C4.03478 10.7254 4.52292 10.9695 5.05173 11.1322L5.18733 11.1729L5.17377 11.3085C5.16021 11.4983 5.21445 11.7017 5.32292 11.8644C5.53987 12.1763 5.91953 12.3254 6.28563 12.2305C6.36699 12.2034 6.44834 12.1763 6.51614 12.1356L10.2314 9.76271C10.4212 9.64068 10.5433 9.46441 10.5839 9.24746C10.6246 9.03051 10.5704 8.8 10.4483 8.62373C10.2314 8.31186 9.85173 8.17627 9.48563 8.27119C9.40428 8.29831 9.32292 8.32542 9.25512 8.3661L7.83139 9.27458C7.60089 9.42373 7.34326 9.5322 7.07207 9.6C5.86529 9.91187 4.60428 9.43729 3.89919 8.42034C3.47885 7.82373 3.30258 7.07797 3.43817 6.34576C3.56021 5.64068 3.99411 5.00339 4.60428 4.62373L8.33309 2.25085C8.5636 2.1017 8.82123 1.99322 9.09241 1.91186C10.2992 1.6 11.5602 2.07458 12.2653 3.09153C12.6992 3.68814 12.8619 4.4339 12.7399 5.1661C12.7128 5.28814 12.6856 5.39661 12.645 5.51864L12.5772 5.73559L12.3873 5.6C11.9399 5.27458 11.4517 5.03051 10.9229 4.8678L10.7873 4.82712L10.8009 4.69153C10.8144 4.50169 10.7602 4.29831 10.6517 4.13559C10.4348 3.82373 10.0551 3.68814 9.68902 3.78305C9.60767 3.81017 9.52631 3.83729 9.45851 3.87797L5.74326 6.25085C5.55343 6.37288 5.43139 6.54915 5.39072 6.7661C5.35004 6.98305 5.40428 7.21356 5.52631 7.38983C5.74326 7.7017 6.12292 7.83729 6.48902 7.74237C6.57038 7.71525 6.65173 7.68814 6.71953 7.64746L8.14326 6.73898C8.37377 6.58983 8.63139 6.48136 8.90258 6.4C10.1094 6.08814 11.3704 6.56271 12.0755 7.57966C12.5094 8.17627 12.6721 8.92203 12.55 9.65424C12.428 10.3593 11.9941 10.9966 11.3839 11.3763L7.65512 13.7492C7.42461 13.8983 7.16699 14.0068 6.8958 14.0881Z" fill="white"></path>
                                    <defs>
                                        <rect width="16" height="16" fill="white"></rect>
                                    </defs></svg>                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Svelte</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><g clipPath="url(#clip0_872_3188)">
                                    <path fillRule="evenodd" clipRule="evenodd" d="M8 0V2C11.3137 2 14 4.68629 14 8C14 11.1453 11.5798 13.7254 8.5 13.9795V15.9846C12.6854 15.7265 16 12.2504 16 8C16 3.58172 12.4183 0 8 0ZM7.5 15.9846V13.9795C6.2188 13.8738 5.05174 13.3655 4.12558 12.5815L2.70769 13.9994C4.00133 15.1415 5.66717 15.8716 7.5 15.9846ZM2.00058 13.2923C0.755509 11.882 0 10.0292 0 8H2C2 9.47685 2.53358 10.8291 3.41847 11.8744L2.00058 13.2923Z" fill="url(#paint0_linear_872_3188)"></path>
                                    <rect x="4.5" y="4.5" width="7" height="7" rx="3.5" stroke="gray-1000" fill="transparent" strokeWidth="2"></rect>
                                </g>
                                    <defs>
                                        <linearGradient id="paint0_linear_872_3188" x1="8.68832" y1="1.98437" x2="1.79792" y2="8.82805" gradientUnits="userSpaceOnUse">
                                            <stop stopColor="#0096FF"></stop>
                                            <stop offset="1" stopColor="#FF1E56"></stop>
                                        </linearGradient>
                                        <clipPath id="clip0_872_3188">
                                            <rect width="16" height="16" fill="white"></rect>
                                        </clipPath>
                                    </defs></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Turborepo</h4>
                                </div>
                            </section>
                        </ul>
                        {/* ////////////////////////////////end Resources////////////////////////////////////// */}
                    </div>

                    <div className='w-[92%]'>
                        <button onClick={rot_block} className='w-full text-[16px] text-[#A1A1A1A1] hover:text-white flex justify-between items-center bg-black h-11 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3'>
                            Solutions
                            <span className=''>
                                <svg viewBox="0 0 16 16" height="16" width="16"><path fill="currentColor" fillRule="evenodd" d="m5.5 1.94.53.53 4.82 4.82a1 1 0 0 1 0 1.42l-4.82 4.82-.53.53L4.44 13l.53-.53L9.44 8 4.97 3.53 4.44 3z" clipRule="evenodd"></path></svg>
                            </span>
                        </button>
                        <ul data-stat='off' className='h-0 scrollbar-thin scrollbar-[#A1A1A1A1] scrollbar- transition-all w-full overflow-hidden'>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M2.5 0.5V0H3.5V0.5C3.5 1.60457 4.39543 2.5 5.5 2.5H6V3V3.5H5.5C4.39543 3.5 3.5 4.39543 3.5 5.5V6H3H2.5V5.5C2.5 4.39543 1.60457 3.5 0.5 3.5H0V3V2.5H0.5C1.60457 2.5 2.5 1.60457 2.5 0.5Z" fill="currentColor"></path>
                                    <path d="M14.5 4.5V5H13.5V4.5C13.5 3.94772 13.0523 3.5 12.5 3.5H12V3V2.5H12.5C13.0523 2.5 13.5 2.05228 13.5 1.5V1H14H14.5V1.5C14.5 2.05228 14.9477 2.5 15.5 2.5H16V3V3.5H15.5C14.9477 3.5 14.5 3.94772 14.5 4.5Z" fill="currentColor"></path>
                                    <path d="M8.40706 4.92939L8.5 4H9.5L9.59294 4.92939C9.82973 7.29734 11.7027 9.17027 14.0706 9.40706L15 9.5V10.5L14.0706 10.5929C11.7027 10.8297 9.82973 12.7027 9.59294 15.0706L9.5 16H8.5L8.40706 15.0706C8.17027 12.7027 6.29734 10.8297 3.92939 10.5929L3 10.5V9.5L3.92939 9.40706C6.29734 9.17027 8.17027 7.29734 8.40706 4.92939Z" fill="currentColor"></path></svg>                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>AI Apps</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fillRule="evenodd" clipRule="evenodd" d="M13.7496 1.89953C13.2746 1.32956 12.571 1 11.8291 1H4.17093C3.42897 1 2.72535 1.32956 2.25037 1.89955L1.07944 3.30467C0.705039 3.75395 0.5 4.32028 0.5 4.90512V5V13.5V15H2H6.04951H9.95049H14H15.5V13.5V5V4.90513C15.5 4.32029 15.295 3.75395 14.9205 3.30466L13.7496 1.89953ZM14 5V4.90513C14 4.67119 13.918 4.44466 13.7682 4.26494L14.9205 3.30466L13.7682 4.26494L12.5973 2.85981C12.4073 2.63182 12.1258 2.5 11.8291 2.5H4.17093C3.87415 2.5 3.5927 2.63182 3.4027 2.85982L2.23178 4.26494C2.08202 4.44465 2 4.67119 2 4.90512V5C2 5.82843 2.67157 6.5 3.5 6.5C4.32843 6.5 5 5.82843 5 5H6.5C6.5 5.82843 7.17157 6.5 8 6.5C8.82843 6.5 9.5 5.82843 9.5 5H11C11 5.82843 11.6716 6.5 12.5 6.5C13.3284 6.5 14 5.82843 14 5ZM14 7.59865C13.5587 7.85391 13.0464 8 12.5 8C11.604 8 10.7997 7.60718 10.25 6.98437C9.70029 7.60718 8.89602 8 8 8C7.10398 8 6.29971 7.60718 5.75 6.98437C5.20029 7.60718 4.39602 8 3.5 8C2.95357 8 2.44126 7.85391 2 7.59865V13.5H6V12C6 10.8954 6.89543 10 8 10C9.10457 10 10 10.8954 10 12V13.5H14V7.59865ZM8.5 12V13.5H7.5V12C7.5 11.7239 7.72386 11.5 8 11.5C8.27614 11.5 8.5 11.7239 8.5 12Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Composable Commerce</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fill="currentColor" fillRule="evenodd" d="M1 1v11.75A2.25 2.25 0 0 0 3.25 15H15v-1.5H3.25a.75.75 0 0 1-.75-.75V1H1Zm13.297 5.013.513-.547-1.094-1.026-.513.547-3.22 3.434-2.276-2.275a1 1 0 0 0-1.414 0L4.22 8.22l-.53.53 1.06 1.06.53-.53L7 7.56l2.287 2.287a1 1 0 0 0 1.437-.023l3.573-3.811Z" clipRule="evenodd"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Marketing Sites</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fillRule="evenodd" clipRule="evenodd" d="M1 3.75C1 2.23122 2.23122 1 3.75 1C5.26878 1 6.5 2.23122 6.5 3.75V5H9.5V3.75C9.5 2.23122 10.7312 1 12.25 1C13.7688 1 15 2.23122 15 3.75C15 5.26878 13.7688 6.5 12.25 6.5H11V9.5H12.25C13.7688 9.5 15 10.7312 15 12.25C15 13.7688 13.7688 15 12.25 15C10.7312 15 9.5 13.7688 9.5 12.25V11H6.5V12.25C6.5 13.7688 5.26878 15 3.75 15C2.23122 15 1 13.7688 1 12.25C1 10.7312 2.23122 9.5 3.75 9.5H5V6.5H3.75C2.23122 6.5 1 5.26878 1 3.75ZM11 5H12.25C12.9404 5 13.5 4.44036 13.5 3.75C13.5 3.05964 12.9404 2.5 12.25 2.5C11.5596 2.5 11 3.05964 11 3.75V5ZM9.5 6.5H6.5V9.5H9.5V6.5ZM11 12.25V11H12.25C12.9404 11 13.5 11.5596 13.5 12.25C13.5 12.9404 12.9404 13.5 12.25 13.5C11.5596 13.5 11 12.9404 11 12.25ZM5 11H3.75C3.05964 11 2.5 11.5596 2.5 12.25C2.5 12.9404 3.05964 13.5 3.75 13.5C4.44036 13.5 5 12.9404 5 12.25V11ZM5 3.75V5H3.75C3.05964 5 2.5 4.44036 2.5 3.75C2.5 3.05964 3.05964 2.5 3.75 2.5C4.44036 2.5 5 3.05964 5 3.75Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Multi-tenant Platforms</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fillRule="evenodd" clipRule="evenodd" d="M1.5 2.5H14.5V12.5C14.5 13.0523 14.0523 13.5 13.5 13.5H2.5C1.94772 13.5 1.5 13.0523 1.5 12.5V2.5ZM0 1H1.5H14.5H16V2.5V12.5C16 13.8807 14.8807 15 13.5 15H2.5C1.11929 15 0 13.8807 0 12.5V2.5V1ZM3.75 5.5C4.16421 5.5 4.5 5.16421 4.5 4.75C4.5 4.33579 4.16421 4 3.75 4C3.33579 4 3 4.33579 3 4.75C3 5.16421 3.33579 5.5 3.75 5.5ZM7 4.75C7 5.16421 6.66421 5.5 6.25 5.5C5.83579 5.5 5.5 5.16421 5.5 4.75C5.5 4.33579 5.83579 4 6.25 4C6.66421 4 7 4.33579 7 4.75ZM8.75 5.5C9.16421 5.5 9.5 5.16421 9.5 4.75C9.5 4.33579 9.16421 4 8.75 4C8.33579 4 8 4.33579 8 4.75C8 5.16421 8.33579 5.5 8.75 5.5Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Web Apps</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M12.25 14.25V10L12.9212 10.1119C14.1403 10.315 15.25 9.37496 15.25 8.13908V7.86092C15.25 6.62504 14.1403 5.68496 12.9212 5.88813L12.25 6V1.75H8.235L8.30764 2.50382C8.41075 3.57386 7.56957 4.5 6.49457 4.5C5.42349 4.5 4.58361 3.58031 4.68058 2.51362L4.75 1.75H0.75V14.25H12.25Z" stroke="currentColor" fill="transparent" strokeWidth="1.5"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Marketplace</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fillRule="evenodd" clipRule="evenodd" d="M14.5 2.5H1.5V5H3V6H1.5V12.5C1.5 13.0523 1.94772 13.5 2.5 13.5H5V12.5H6V13.5H13.5C14.0523 13.5 14.5 13.0523 14.5 12.5V6H14V5H14.5V2.5ZM1.5 1H0V2.5V12.5C0 13.8807 1.11929 15 2.5 15H13.5C14.8807 15 16 13.8807 16 12.5V2.5V1H14.5H1.5ZM5 9V7.5H6V9H5ZM5 11.5V10H6V11.5H5ZM6.5 5V6H8V5L6.5 5ZM4 5V6H5.5V5H4ZM9 5V6H10.5V5H9ZM11.5 5V6H13V5H11.5Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Temlates</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path fillRule="evenodd" clipRule="evenodd" d="M15.7329 1.15933L15.9807 0.0192871L14.8407 0.267122L5.22524 2.35744C3.79024 2.66939 2.66939 3.79024 2.35744 5.22523L0.267122 14.8407L0.0192871 15.9807L1.15933 15.7329L10.7748 13.6426C12.2098 13.3306 13.3306 12.2098 13.6426 10.7748L15.7329 1.15933ZM5.54388 3.8232L14.0193 1.98072L12.1768 10.4561C11.9896 11.3171 11.3171 11.9896 10.4561 12.1768L1.98072 14.0193L3.8232 5.54388C4.01037 4.68288 4.68288 4.01037 5.54388 3.8232ZM9 8C9 8.55229 8.55229 9 8 9C7.44772 9 7 8.55229 7 8C7 7.44772 7.44772 7 8 7C8.55229 7 9 7.44772 9 8ZM10.5 8C10.5 9.38072 9.38072 10.5 8 10.5C6.61929 10.5 5.5 9.38072 5.5 8C5.5 6.61929 6.61929 5.5 8 5.5C9.38072 5.5 10.5 6.61929 10.5 8Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Partner</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M12.798 1.24199L13.3283 1.77232L14.0567 1.04389L13.1398 0.574402L12.798 1.24199ZM9.95705 4.0829L9.42672 3.55257L9.95705 4.0829ZM6.5844 6.95555L7.11473 7.48588L7.46767 7.13295L7.27837 6.67111L6.5844 6.95555ZM1.49995 12.04L2.03027 12.5703L2.03028 12.5703L1.49995 12.04ZM1.49994 14.54L0.969615 15.0703H0.969615L1.49994 14.54ZM3.99995 14.54L4.53028 15.0703L3.99995 14.54ZM9.10147 9.43848L9.37633 8.74066L8.91883 8.56046L8.57114 8.90815L9.10147 9.43848ZM14.7848 3.25519L15.4568 2.92229L14.9931 1.98617L14.2544 2.72486L14.7848 3.25519ZM11.9571 6.0829L11.4267 5.55257L11.9571 6.0829ZM10.5428 6.0829L11.0732 5.55257L11.0732 5.55257L10.5428 6.0829ZM9.95705 5.49711L9.42672 6.02744L9.42672 6.02745L9.95705 5.49711ZM12.2676 0.711655L9.42672 3.55257L10.4874 4.61323L13.3283 1.77232L12.2676 0.711655ZM10.7499 1.5C11.3659 1.5 11.9452 1.64794 12.4562 1.90957L13.1398 0.574402C12.4221 0.206958 11.6091 0 10.7499 0V1.5ZM6.99994 5.25C6.99994 3.17893 8.67888 1.5 10.7499 1.5V0C7.85045 0 5.49994 2.3505 5.49994 5.25H6.99994ZM7.27837 6.67111C7.09913 6.23381 6.99994 5.75443 6.99994 5.25H5.49994C5.49994 5.95288 5.63848 6.62528 5.89043 7.23999L7.27837 6.67111ZM6.05407 6.42522L0.969615 11.5097L2.03028 12.5703L7.11473 7.48588L6.05407 6.42522ZM0.969616 11.5097C-0.0136344 12.4929 -0.013635 14.0871 0.969615 15.0703L2.03027 14.0097C1.63281 13.6122 1.63281 12.9678 2.03027 12.5703L0.969616 11.5097ZM0.969615 15.0703C1.95287 16.0536 3.54703 16.0536 4.53028 15.0703L3.46962 14.0097C3.07215 14.4071 2.42774 14.4071 2.03027 14.0097L0.969615 15.0703ZM4.53028 15.0703L9.6318 9.96881L8.57114 8.90815L3.46962 14.0097L4.53028 15.0703ZM10.7499 9C10.2637 9 9.80071 8.90782 9.37633 8.74066L8.82661 10.1363C9.4232 10.3713 10.0724 10.5 10.7499 10.5V9ZM14.4999 5.25C14.4999 7.32107 12.821 9 10.7499 9V10.5C13.6494 10.5 15.9999 8.1495 15.9999 5.25H14.4999ZM14.1127 3.5881C14.3604 4.08811 14.4999 4.65168 14.4999 5.25H15.9999C15.9999 4.4156 15.8047 3.62465 15.4568 2.92229L14.1127 3.5881ZM12.4874 6.61323L15.3151 3.78552L14.2544 2.72486L11.4267 5.55257L12.4874 6.61323ZM10.0125 6.61323C10.6959 7.29665 11.804 7.29665 12.4874 6.61323L11.4267 5.55257C11.3291 5.6502 11.1708 5.6502 11.0732 5.55257L10.0125 6.61323ZM9.42672 6.02745L10.0125 6.61323L11.0732 5.55257L10.4874 4.96678L9.42672 6.02745ZM9.42672 3.55257C8.7433 4.23599 8.7433 5.34403 9.42672 6.02744L10.4874 4.96678C10.3898 4.86915 10.3897 4.71086 10.4874 4.61323L9.42672 3.55257Z" fill="currentColor"></path></svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Platform Engineers</h4>
                                </div>
                            </section>
                            <section className='p-1 flex items-center w-full cursor-pointer transition-all text-[#A1A1A1A1] hover:text-white hover:bg-[#352d2d] rounded-md'>
                                <svg height="16" strokeLinejoin="round" className='rounded-sm h-8.75 w-8.75' viewBox="-6 -1 30 16" width='16'><path d="M9 7L12.5 2.5" stroke="#E5484D" className='stroke-[color(display-p3 0.8980 0.2824 0.3020)]' strokeOpacity='1' strokeWidth="1.5"></path>
                                    <path d="M10.5 9.5L15.75 10.5" stroke="#52AEFF" className='stroke-[color(display-p3 0.3216 0.6824 1.0000)]' strokeOpacity='1' strokeWidth="1.5"></path>
                                    <path d="M10 8L15.75 6" stroke="#45DEC4" className='stroke-[color(display-p3 0.2706 0.8706 0.7686)]' strokeOpacity='1' strokeWidth="1.5"></path>
                                    <path fillRule="evenodd" clipRule="evenodd" d="M6.14568 3.56625L7 2L7.85432 3.56625L12.1818 11.5L13 13H11.2914H2.70863H1L1.81818 11.5L3.31818 8.75H0V7.25H4.13636L6.14568 3.56625ZM3.52681 11.5L7 5.13249L10.4732 11.5H3.52681Z" fill="currentColor"></path>
                                </svg>
                                <div className='ml-2'>
                                    <h4 className=' w-full transition-all'>Design Engineers</h4>
                                </div>
                            </section>
                        </ul>
                        {/* ////////////////////////////////end Solutions////////////////////////////////////// */}
                    </div>

                    <div className='w-[92%]'>
                        <Link href='' className='w-full text-[16px] text-[#A1A1A1A1] hover:text-white flex justify-between items-center bg-black py-2 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3'>
                            Pricing
                        </Link>
                    </div>
                    <div className='w-[92%]'>
                        <Link href='' className='w-full text-[16px] text-[#A1A1A1A1] hover:text-white flex justify-between items-center bg-black py-2 px-2.5 rounded-md text-sm hover:bg-[#352d2d] mt-3'>
                            Contact
                        </Link>
                    </div>
                    <div className='w-[92%] flex items-center justify-between border border-t border-b py-8 mt-3 px-2.5 border-l-0 border-r-0 border-[#A1A1A1A1] text-[#A1A1A1A1]'>
                        <span className='hover:text-white'>Theme</span>
                        <DarkLight/>
                    </div>
                </section>
            </div>
        </>
    )
}
